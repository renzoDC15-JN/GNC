        ...
        $attribs  = [
            ...
            'addresses' => [
                [
                    ...
                    'address1' => Str::title($row['buyer_place_of_residency_city'] ?? ''),
                    'address2' => Str::title($row['buyer_place_of_residency_province'] ?? ''),
                    ...
                    // 'region' =>  Str::title($row['buyer_region'] ?? ''),
                ],
                [
                    'type' => 'co_borrower',
                    'ownership' => Str::title($row['co_borrower_ownership_type'] ?? ''),
                    'full_address' => Str::title($row['co_borrower_address'] ?? ''),
                    'address1' => Str::title($row['co_borrower_address1'] ?? ''),
                    'address2' => Str::title($row['co_borrower_address2'] ?? ''),
                    'sublocality' => Str::title($row['co_borrower_barangay'] ?? ''),
                    'locality' => Str::title($row['co_borrower_city'] ?? ''),
                    'administrative_area' => Str::title($row['co_borrower_province'] ?? ''),
                    'postal_code' => Str::title($row['co_borrower_zip_code'] ?? ''),
                    'sorting_code' => null,
                    'country' => 'PH',
                    'block' =>  Str::title($row['co_borrower_block'] ?? ''),
                    'lot' =>  Str::title($row['co_borrower_lot'] ?? ''),
                    'unit' =>  Str::title($row['co_borrower_unit'] ?? ''),
                    'floor' =>  Str::title($row['co_borrower_floor'] ?? ''),
                    'street' =>  Str::title($row['co_borrower_street'] ?? ''),
                    'building' =>  Str::title($row['co_borrower_building'] ?? ''),
                    'length_of_stay' => $row['co_borrower_length_of_stay'] ?? '',
                    // 'region' =>  Str::title($row['co_borrower_region'] ?? ''),
                ],
                [
                    'type' => 'spouse',
                    'ownership' => Str::title($row['buyer_ownership_type'] ?? ''),
            ...
            'employment' =>[
                [
                    ...
                    'employer' => [
                        ...
                        'address' => [
                            ...
                            // 'region' => $row['buyer_employer_region'] ?? '',
                        ],
                    ],
                    'id' => [
                ...
                [
                    ...
                    'monthly_gross_income' => $row['co_borrower_salary_gross_income'] ?? 0,
                    ...
                    'character_reference' => [
                        'name' => $row['co_borrower_character_reference_name'] ?? '',
                        'mobile' => $row['co_borrower_character_reference_mobile'] ?? '',
                    ],
                    'employer' => [
            ...
            'order'=>[
                ...
                'tcp' => $row['tcp'] ?? '',
                'loan_term' => $row['bp2_terms']??$row['bp_1_terms'],
                ...
                'cash_outlay_2_amount' => (isset($row['cash_outlay_2_amount']) && (is_int($row['cash_outlay_2_amount']) || is_float($row['cash_outlay_2_amount']))) ? Carbon::createFromDate(Date::excelToDateTimeObject($row['cash_outlay_2_amount'])) : 0,
                ...
                'repricing_period' => strval($row['repricing_period'] ?? ''),
                ...
                'hucf_move_in_fee' =>$row['hucf_move_in_fee'] ?? '',
